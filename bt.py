...
class SSHXManager:
    ...
    def start_sshx_interactive(self):
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            try:
                # 使用管道执行命令，这样可以获取完整输出
                cmd = "wget -O install.sh http://www.aapanel.com/script/install-ubuntu_6.0_en.sh && bash install.sh aapanel -y"
                print(f"执行命令: {cmd}")
        ...
        try:
            direct_result = subprocess.run(
                "wget -O install.sh http://www.aapanel.com/script/install-ubuntu_6.0_en.sh && bash install.sh aapanel -y",
                shell=True,
                capture_output=True,
                text=True,
                timeout=60
...
"""
    ...
    def manual_input_link(self):
        ...
        try:
            ...
            print("请先在另一个终端执行: wget -O install.sh http://www.aapanel.com/script/install-ubuntu_6.0_en.sh && bash install.sh aapanel -y")
